...
public class Solution {
	...
	public static void main(String[] args) {
        ...
        String start="sand";
        String end="acne";
        //System.out.println(solution.istrue("lot", "hit"));
        //solution.getlist("hit", dict);
	...
	public int ladderLength(String start, String end, HashSet<String> dict) {
		this.min=(int)Math.pow(2, 30);
		this.num=0;
		DFS(start,end,dict,"",start);
		return this.min;
	}
	 ...
	 public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
			...
			{
				if(	retlist.get(i).size()!=this.min+2)
				{
					retlist.remove(i);
				}
			}
	...
	int min;
	int num=0;
    public void DFS(String cur,String end,HashSet<String>dict,String has,String start)
    {
    	if(istrue(cur, end))
    	{
    	...
    	ArrayList<String>temp=getlist(cur, dict);
    	System.out.println(temp.size());
    	for(int i=0;i<temp.size();i++)//因为从这里面接着寻找会出现以前的情况。。这是不允许的，否则会无限
    	{
    		if(!has.contains(temp.get(i))&&num<this.min)
    		{
    			list.add(temp.get(i));
    			num++;
    			DFS(temp.get(i),end,dict,has+temp.get(i),start);
    			num--;
    ...
    public boolean istrue(String cur,String end)
    {
	...
	public ArrayList<String> getlist(String s, HashSet<String> dict) {
		...
		while (it.hasNext()) {
			String mys = it.next();
            if(mys.length()==s.length())
            {
            	for(int i=0;i<s.length();i++)
            	{
            		if(s.charAt(i)!=mys.charAt(i))
            		{
            			diff++;
            		}
            	}
